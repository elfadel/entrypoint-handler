...
int build_ip4_tuple(struct ip4_tuple *ip_tuple, struct sock *skp) {
	...
	ip_tuple->sport = bpf_ntohs(src_port);
	ip_tuple->dport = bpf_ntohs(dst_port);

	return 1;
}

SEC("kprobe/sys_listen")
int kprobe__sys_listen(struct pt_regs *ctx) {
...
SEC("kretprobe/sys_listen")
int kretprobe__sys_listen(struct pt_regs *ctx) {
